...
def handler(event, context):
    ...
    try:
        ...
        print(json.dumps({"event": "debrief.memories_fetched", "count": len(memories)}))
    except Exception as e:
        print(json.dumps({"event": "debrief.memory_error", "error": str(e)}))
        memories = []

    try:
        text = weekly_debrief(goal, workouts, biometrics, memories)
